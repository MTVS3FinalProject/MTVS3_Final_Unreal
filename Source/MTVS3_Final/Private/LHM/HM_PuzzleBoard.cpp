...
{
	AHM_PuzzlePiece* PuzzlePiece = Cast<AHM_PuzzlePiece>(OtherComp);
	if(PuzzlePiece)
	{
		FString PieceCompTag = OtherComp->ComponentTags[0].ToString();

		for(int32 i = 1; i <= 9; i++)
		{
			FString PieceName = *FString::Printf(TEXT("Piece%d"), i);
			if(PieceCompTag == PieceName)
			{
				//SetVisibility(true);
				PuzzlePiece->ApplyBounceEffect();
			}	
		}
